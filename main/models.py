...
def validate_phone_number(value):
    prefixes = ['980', '981', '982', '984', '985', '986', '970', '974', '975', '976']
    if not any(value.startswith(prefix) for prefix in prefixes) or not value.isdigit() or len(value) != 10:
        raise ValidationError('Invalid phone number.')

from enum import Enum

class StatusEnum(Enum):
    GATE_PASS = 'Pass Issued'
    ENTERED = 'Entered'
    EXITED = 'Exited'
...
def map_button_to_status(button_value):
    if button_value == ButtonEnum.ENTRY.value:
        return StatusEnum.GATE_PASS.value
    elif button_value == ButtonEnum.EXIT.value:
        return StatusEnum.GATE_PASS.value
    else:
        return StatusEnum.ENTERED.value
...
class FormData(models.Model):
    ...
    id_number = models.CharField(max_length=10, validators=[validate_phone_number])

    # id_number = models.CharField(max_length=100, default ='023456')
    ...
    current_status = models.CharField(max_length=100, default = StatusEnum.GATE_PASS.value, choices=STATUS_CHOICES,blank=True)
    entry_button = models.CharField(max_length=100, default=ButtonEnum.ENTRY.value,blank=True)
    exit_button = models.CharField(max_length=100, default=ButtonEnum.EXIT.value,blank=True)

	#added Sunday 4pm
    group = models.CharField(max_length=100, blank=True ) 
    assigned_office = models.CharField(max_length=100, blank=True ) 

    date_of_entry = models.CharField(max_length=20,blank=True,null=True)

    # date_of_entry = models.DateTimeField(blank=True,null=True)  # Entry time
    date_of_exit = models.CharField(blank=True, null=True) 
    duration_of_stay = models.CharField(max_length=50, blank=True)


    #primary_key = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False, unique=True)
    # display_num_of_people = models.BooleanField(default=False)
    num_of_add_people = models.IntegerField(default=0, null=True, blank=True)
    # people_names = models.CharField(max_length=255,blank=True, null=True)

    additional_people = models.BooleanField(default=False)
    additional_people_names = models.CharField(max_length=255, blank=True, null=True)
    ...
    @property
    def status(self):
        return map_button_to_status(self.entry_button)

    # def save(self, *args, **kwargs):
    #     if self.date_of_entry and self.date_of_exit:
    #         # Calculate the duration of stay
    #         duration = self.date_of_exit - self.date_of_entry
    #         hours, remainder = divmod(duration.seconds, 3600)
    #         minutes, seconds = divmod(remainder, 60)
    #         self.duration_of_stay = f"{hours} hours {minutes} minutes {seconds} seconds"
    #     else:
    #         self.duration_of_stay = "N/A"
